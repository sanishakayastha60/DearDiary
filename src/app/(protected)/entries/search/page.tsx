...
export default function SearchBar() {
  ...
  async function handleSearch(value: string) {
    ...
    try {
      const response = await fetch(
        `/api/entries/search?q=${encodeURIComponent(value)}`,
  ...
  return (
    <div className="ml-[10vw] w-[75%] h-full p-2 rounded-xl flex flex-col gap-2">
      <h1 className="text-2xl text-center font-rubik-doodle-shadow font-bold uppercase mt-8">
        Revisit a memory . . .
      </h1>
      <div className="mt-6 flex w-full max-w-xl items-center rounded-2xl border border-gray-400 px-4 py-2 shadow-sm transition-all focus-within:border-gray-600 focus-within:shadow-md">
        ...
        <input
          ...
          className="w-full bg-transparent font-kalam font-bold py-2 text-sm text-gray-800 outline-none placeholder:text-gray-400"
        />
      </div>
      ...
      {entries.length > 0 && (
        <div className="max-h-[50vh] overflow-y-auto pr-2 [scrollbar=width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          {entries.map((entry) => (
            <div
              key={entry.id}
              className="w-full min-w-0 overflow-hidden p-4 rounded-xl bg-card text-card-foreground m-2"
            >
              <h2 className="text-xl uppercase font-kalam font-bold">
                {entry.title}
              </h2>
